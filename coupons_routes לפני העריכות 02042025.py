...
def submit_buyme_coupon_urls():
    ...
    return render_template(
        "add_coupons.html", form=form, coupons=coupons, companies=companies
    )
...
def review_usage_findings():
    ...
    if request.method == "GET":
        ...
        current_app.logger.info(
            f"[DEBUG] Fetched {len(all_user_coupons)} active coupons for user {current_user.id}"
        )
        ...
        for i, row in enumerate(usage_list):
            ...
            try:
                ...
                current_app.logger.warning(
                    f"[DEBUG] row-{i} => failed to convert '{amount_str}' to float. Using 0.0"
                )
