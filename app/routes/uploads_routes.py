...
def add_coupons_bulk():
        ...
        try:
            ...
            for idx, coupon_entry in enumerate(form.coupons.entries):
                ...
                if coupon_form.company_id.data == "other":
                    ...
                    try:
                        ...
                        if company:
                            ...
                            flash(
                                f"חברה עם ID {company_id} לא נמצאה בקופון #{idx + 1}.",
                                "danger",
                            )
                            continue
                    except ValueError:
                ...
                if coupon_form.tag_id.data == "other":
                    ...
                    try:
                        ...
                        if tag:
                            ...
                            flash(
                                f"תגית עם ID {tag_id} לא נמצאה בקופון #{idx + 1}.",
                                "danger",
                            )
                            continue
                    except ValueError:
                ...
                coupon_data = {
                    ...
                    "ערך מקורי": value,
                    "עלות": cost,
                    "תאריך תפוגה": coupon_form.expiration.data.strftime("%Y-%m-%d")
                    if coupon_form.expiration.data
                    else "",
                    "קוד לשימוש חד פעמי": coupon_form.is_one_time.data,
                    "מטרת הקופון": coupon_form.purpose.data.strip()
                    if coupon_form.is_one_time.data and coupon_form.purpose.data
                    else "",
                    "תיאור": coupon_form.description.data.strip()
                    if coupon_form.description.data
                    else "",
            ...
            if new_coupons_data:
                ...
                current_app.logger.info(
                    "All coupons successfully processed and imported."
                )
                # log_user_activity("add_coupons_bulk_submit")
                return redirect(url_for("transactions.show_coupons"))
            else:
                current_app.logger.info(
                    "No new coupons were added, so no export or import was made."
                )
    ...
    return render_template(
        "add_coupons_bulk.html", form=form, companies=companies, tags=tags
    )
