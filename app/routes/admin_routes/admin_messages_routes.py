...
admin_messages_bp = Blueprint(
    "admin_messages_bp", __name__, url_prefix="/admin/messages"
)
...
def manage_admin_messages():
    ...
    if request.method == "POST" and form.validate_on_submit():
        ...
        new_message = AdminMessage(
            message_text=message_text, link_url=link_url, link_text=link_text
        )
    ...
    return render_template(
        "admin/admin_manage_messages.html", form=form, messages=messages
    )
...
def delete_multiple_admin_messages():
    ...
    AdminMessage.query.filter(AdminMessage.id.in_(message_ids)).delete(
