...
if not SUPABASE_URL or not SUPABASE_KEY:
    raise ValueError("❌ Error: SUPABASE_URL and SUPABASE_ANON_KEY must be set in the .env file")
...
def fetch_users() -> pd.DataFrame:
    ...
    sample_data = [
        {"id": 26, "email": "itayk93@yahoo.com", "first_name": "איתיטסט1", "last_name": "להלה"},
        {"id": 29, "email": "yahavtr07@gmail.com", "first_name": "יהב", "last_name": "טרבלסי"},
        {"id": 35, "email": "avso1981@gmail.com", "first_name": "אבי", "last_name": "סובול"},
        {"id": 40, "email": "orendi1978@gmail.com", "first_name": "Oren", "last_name": "Dichterman"},
        {"id": 39, "email": "ordona1995@gmail.com", "first_name": "אליאור", "last_name": "אזואלוס"},
        {"id": 41, "email": "eladi111@hotmail.com", "first_name": "אלעד", "last_name": "דבש"},
        {"id": 1, "email": "itayk93@gmail.com", "first_name": "איתי", "last_name": "קרקסון"},
        {"id": 42, "email": "tjuacwmobiyrjdaxcc@poplk.com", "first_name": "kvkv", "last_name": "kkf"},
        {"id": 28, "email": "arielfrid20011@gmail.com", "first_name": "אריאל", "last_name": "פריד"},
        {"id": 36, "email": "david.ruimy1@gmail.com", "first_name": "דוד", "last_name": "רוימי"}
    ]
...
def plot_status_distribution(df: pd.DataFrame):
    ...
    fig = px.pie(status_counts, names="status_category", values="percentage",
...
def plot_discount_percentage_by_company(df: pd.DataFrame, top_n=5):
    ...
    fig = px.bar(discount_avg, x="company", y="avg_discount",
...
def plot_value_vs_cost(df: pd.DataFrame, company_filter: list = None):
    ...
    fig.update_layout(title="Comparison of Coupon Value vs. Cost",
                      xaxis_title="Value",
                      yaxis_title="Cost")
    set_layout(fig)
    return fig
...
def plot_usage_type_segmentation(df: pd.DataFrame):
    ...
    usage_counts["Usage Type"] = usage_counts["is_one_time"].apply(lambda x: "One-time" if x else "Multi-use")
    fig = px.bar(usage_counts, x="Usage Type", y="count",
...
def plot_top_discounts(df: pd.DataFrame):
    ...
    top_discount = top_discount.sort_values("discount_percentage", ascending=False).head(10)
    fig = px.bar(top_discount, x="company", y="discount_percentage",
                 title="Top 10 Companies by Highest Coupon Discount",
                 labels={"company": "Company", "discount_percentage": "Max Discount (%)"},
                 template="plotly_white",
                 color="discount_percentage", color_continuous_scale="Viridis")
    fig.update_layout(xaxis_tickangle=45)
    set_layout(fig)
    return fig
...
def plot_usage_analysis(df: pd.DataFrame):
    ...
    fig = px.pie(names=["Used Value", "Remaining Value"],
...
def plot_availability_and_sale(df: pd.DataFrame):
    ...
    y_cols = [col for col in ["For Sale", "Not For Sale"] if col in availability.columns]
    if not y_cols:
        return None
    fig = px.bar(availability, x="is_available", y=y_cols,
...
def plot_expiration_segmentation(df: pd.DataFrame):
    ...
    today = pd.Timestamp.now()
    df = df.copy()
    df["expiration_status"] = df["expiration"].apply(lambda x: "Expired" if pd.notnull(x) and x < today 
                                                     else ("About to Expire" if pd.notnull(x) and x <= today + pd.Timedelta(days=7) 
                                                           else "Valid"))
    status_counts = df["expiration_status"].value_counts().reset_index()
    status_counts.columns = ["expiration_status", "count"]
    fig = px.bar(status_counts, x="expiration_status", y="count",
...
def plot_monthly_highest_discount(df: pd.DataFrame):
    ...
    fig = px.line(monthly_max, x="month", y="discount_percentage",
...
def generate_overall_kpis(df: pd.DataFrame):
    ...
    ratio = (df["value"] / df["cost"]).mean() if "value" in df.columns and "cost" in df.columns and (df["cost"] != 0).all() else 0
    if "company" in df.columns:
        seg = df["company"].value_counts().head(5)
        seg_html = "<ul class='list-unstyled mb-0'>" + "".join([f"<li><strong>{comp}</strong>: {cnt}</li>" for comp, cnt in seg.items()]) + "</ul>"
    else:
        seg_html = "N/A"
    html = f"""
    <div class="row text-center my-3">
...
def build_detailed_dashboard() -> str:
    ...
    if users_df.empty:
        ...
        for _, row in users_df.iterrows():
            selected = "selected" if str(row["id"]) in request.args.getlist('user_id') else ""
...
"""
    ...
    for func, title in graph_functions:
        ...
        if fig is not None:
            graph_div = plotly_plot(fig, include_plotlyjs=False, output_type='div', config={'responsive': True})
            graph_divs += f"""
