...
def manage_coupon_tags():
    ...
    if request.method == 'POST':
        ...
        if "update_tag" in request.form:
            ...
            logger.info(f"Updating category for coupon {submitted_coupon_id} to tag {tag_id}.")
            ...
            try:
                ...
                db.session.rollback()
                logger.error(f"Error updating category for coupon {submitted_coupon_id}: {e}")
...
def update_multiple_tags():
    ...
    # Parse coupon IDs
    try:
        coupon_ids = [int(id_str) for id_str in coupon_ids_str.split(',') if id_str.strip()]
    except ValueError:
    ...
    try:
        ...
        db.session.commit()
        flash(f"{updated_count} קופונים עודכנו בהצלחה עם הקטגוריה '{tag.name}'!", "success")
