...
from app.helpers import send_coupon_purchase_request_email, get_coupon_data, get_geo_location, get_public_ip
...
def log_user_activity(action, coupon_id=None):
    try:
        ...
        current_app.logger.info(f"[LOG] Action='{action}', coupon_id={coupon_id}, user_id={current_user.id if current_user else None}")
        current_app.logger.info(f"[LOG] IP={ip_address}, GEO={geo_data}, UA={user_agent[:50]}")
...
def marketplace():
    ...
    try:
        ...
        requested_coupon_ids = [
            t.coupon_id for t in Transaction.query.filter_by(buyer_id=current_user.id).all()
        ]
...
def offer_coupon_selection(request_id):
    ...
    current_app.logger.info(f"[DEBUG] Entering offer_coupon_selection => request_id={request_id}")
    ...
    current_app.logger.info(f"[DEBUG] Found {len(seller_coupons)} existing coupons for user {current_user.id}")
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('marketplace.offer_coupon_process', request_id=request_id))
    else:
        ...
        return redirect(url_for('marketplace.offer_coupon_selection', request_id=request_id))
...
def offer_coupon_process(request_id):
    ...
    from app.forms import OfferCouponForm
    form = OfferCouponForm()
    current_app.logger.info(f"[DEBUG] offer_coupon_process => POST data = {request.form}")
    ...
    current_app.logger.info(f"[DEBUG] offer_choice={offer_choice}, seller_message={seller_message}")
