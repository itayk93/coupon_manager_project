...
def transfer_coupons(source_tag_id):
    ...
    other_tags = [
        (tag.id, tag.name) for tag in Tag.query.filter(Tag.id != source_tag_id).all()
    ]
    if not other_tags:
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(
            f'{count} קופונים הועברו בהצלחה מהתגית "{source_tag.name}" לתגית "{target_tag.name}".',
            "success",
        )
        return redirect(url_for("admin_tags_bp.manage_tags"))
...
def update_coupon_transactions():
    ...
    if df is not None:
        ...
        CouponTransaction.query.filter_by(
            coupon_id=coupon.id, source="Multipass"
        ).delete()
...
def manage_feature_access():
    ...
    if request.method == "POST":
        ...
        new_feature = FeatureAccess(feature_name=feature_name, access_mode=access_mode)
